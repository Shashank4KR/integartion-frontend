...
export default function AccountantInvoicesPage() {
  ...
  const loadInvoices = async () => {
    ...
    try {
      ...
      (Array.isArray(studentsRes) ? studentsRes : []).forEach((s: any) => {
        ...
        const name = `${first} ${last}`.trim() || s.name || s.username || "Student";
        const adm = s.admission_no || s.admissionNo || String(s.id);
        let cls = "General";
        if (s.class_name) {
          cls = s.class_name;
        } else if (s.class_id && classMap.has(String(s.class_id))) {
          cls = classMap.get(String(s.class_id))!;
        } else if (s.class_ && typeof s.class_ === "object") {
          cls = `${s.class_.class_name || "Class"} ${s.class_.section ? `- ${s.class_.section}` : ""}`.trim();
        }
        studentMap.set(String(s.id), { name, admissionNo: adm, classGrade: cls });
        studentMap.set(adm, { name, admissionNo: adm, classGrade: cls });
      });
  ...
  return (
    <RoleDashboardLayout config={ROLE_CONFIGS.accountant}>
      ...
      {toast.open && (
        <div className="fixed bottom-6 right-6 z-50 rounded-lg bg-slate-900 px-4 py-3 text-sm text-white shadow-xl flex items-center gap-2">
