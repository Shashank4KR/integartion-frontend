...
export default function RoleInboxPage({ role, kind }: { role: Role; kind: InboxKind }) {
  ...
  const load = async () => {
    ...
    try {
      if (kind === "notifications") {
        ...
        combined.sort((a, b) => {
          const dateA = new Date((a as any).created_at || (a as any).sent_on || (a as any).sent_at || 0).getTime();
          const dateB = new Date((b as any).created_at || (b as any).sent_on || (b as any).sent_at || 0).getTime();
          return dateB - dateA;
        });
        setItems(combined);
      } else {
  ...
  return (
    <RoleDashboardLayout config={ROLE_CONFIGS[role]}>
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">{config.title}</h1>
            <p className="mt-1 text-sm text-slate-500">Live updates from the communication service.</p>
          </div>
          <button
            type="button"
            onClick={() => void load()}
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          {loading ? (
            <p className="p-6 text-sm text-slate-500">Loading {kind}…</p>
          ) : error ? (
            <p role="alert" className="m-5 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              {error}
            </p>
          ) : items.length === 0 ? (
            <div className="grid min-h-64 place-items-center p-8 text-center">
              <div>
                <span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-violet-100 text-violet-700">
                  <Icon className="h-6 w-6" />
                </span>
                <p className="mt-4 text-sm text-slate-500">{config.emptyLabel}</p>
              </div>
            </div>
          ) : (
            <ul>
              {items.map((item, index) => (
                <li
                  key={text(item, ["id", "message_id", "notification_id"], String(index))}
                ...
                >
                  <p className="text-sm font-semibold text-slate-900">
                    {text(
                      item,
                      kind === "messages"
                        ? ["subject", "title", "sender_name"]
                        : ["title", "subject", "type"],
                      kind === "messages" ? "Message" : "Notification"
                    )}
                  </p>
